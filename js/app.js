let DATA = {
    ...
    kwColors: { "화상": "#ff0000", "출혈": "#ff5900", "진동": "#ffc800", "파열": "#01ff09", "침잠": "#00e1ff", "호흡": "rgb(0, 4, 255)", "충전": "#820095", "혈찬": "#6a0505", "탄환": "#3c3c3c", "버림": "#4a2c00"}
};

let slots = [], activeIdx = 0, giftSortAsc = false, collapsedCats = new Set();
let isLCol = false, isRCol = false, isResCol = false;
...
async function loadAllData() {
    try {
        ...
        document.getElementById('active-slot-name-display').innerText = "DATA LOAD FAILED";
    }
}
...
function renameSlot(i) {
    const n = prompt("전략 이름을 입력하세요:", slots[i].name);
...
function renderSlots() {
    ...
    slots.forEach((s, i) => {
        ...
        div.className = `group flex items-center p-2 rounded cursor-pointer mb-1 transition ${activeIdx === i ? 'active-slot bg-stone-900' : 'text-stone-500 hover:bg-stone-800'}`;
        div.draggable = true;
        ...
        div.innerHTML = `<i class="fas fa-grip-vertical text-[9px] mr-2 text-stone-700 pointer-events-none"></i><span class="flex-1 truncate font-bold pointer-events-none">${s.name}</span><button onclick="event.stopPropagation(); deleteSlot(${i})" class="opacity-0 group-hover:opacity-100 px-1 text-stone-600 hover:text-red-500 transition"><i class="fas fa-trash"></i></button>`;
...
function getStats() {
    ...
    s.party.forEach((p, i) => {
        ...
        const id = DATA.identities[p.sinner].find(x => x.name === p.idName);
        if(!id) return;
        const uKws = new Set();
        ["s1","s2","s3"].forEach(sk => { if(p[sk+"_count"] > 0 && id[sk].keywords) id[sk].keywords.forEach(k => { if(!excluded.includes(k)) uKws.add(k); }); });
...
function checkGiftStatus(g, p, idx, stats) {
    ...
    s.gifts.forEach(id => {
        const giftData = DATA.allGifts.find(x => x.id === id);
        if (giftData?.replaces) giftData.replaces.forEach(rid => replacedIds.add(rid));
    });

    if (!s.gifts.includes(g.id) || replacedIds.has(g.id) || !p.idName) return 0;
    const id = DATA.identities[p.sinner]?.find(x => x.name === p.idName);
    if (!id) return 0;

    const idFacs = Array.isArray(id.faction) ? id.faction : (id.faction ? [id.faction] : []);
    const hasAnyKw = (kwList) => [id.s1, id.s2, id.s3].some(sk => sk && sk.keywords && kwList.some(k => sk.keywords.includes(k)));
    const getGlobalKwCount = (kwList) => s.party.filter(m => {
        const mId = DATA.identities[m.sinner]?.find(x => x.name === m.idName);
        return mId && [mId.s1, mId.s2, mId.s3].some(sk => sk && sk.keywords && kwList.some(k => sk.keywords.includes(k)));
    }).length;

    let isBase = false;
    const targetKws = g.kws || (g.kw ? [g.kw] : []);
    const targetFacs = g.factions || (g.faction ? [g.faction] : []);
    ...
    switch(g.type) {
        ...
        case "total_kw_multi": isBase = getGlobalKwCount(targetKws) >= (g.min || 1) && hasAnyKw(targetKws); break;
        ...
        case "faction_list": isBase = targetFacs.some(f => idFacs.includes(f)) && targetFacs.some(f => stats.totFac[f] >= (g.min || 1)); break;
        case "sk_type": let sc=0; ["s1","s2","s3"].forEach(sk => { if(p[sk+'_count']>0 && id[sk].type === g.atk) sc++; }); isBase = sc >= g.min; break;
        ...
        case "pos_sk_type": 
            const isPos = Array.isArray(g.targetSlot) ? g.targetSlot.includes(idx) : idx === g.targetSlot;
            let psc=0; ["s1","s2","s3"].forEach(sk => { if(p[sk+'_count']>0 && id[sk].type === g.atk) psc++; });
            isBase = isPos && psc >= g.min; break;
    }
    if (!isBase) return 0;

    if (g.extras && Array.isArray(g.extras)) {
        const isExtra = g.extras.some(ex => {
            if (ex.type === "faction") return idFacs.includes(ex.val);
            if (ex.type === "faction_list") return ex.factions.some(f => idFacs.includes(f));
            if (ex.type === "sk_type") {
                let ec=0; ["s1","s2","s3"].forEach(sk => { if(p[sk+'_count']>0 && id[sk].type === ex.atk) ec++; });
                return ec >= (ex.min || 1);
            }
...
function renderParty() {
    ...
    s.party.forEach((p, i) => {
        ...
        const giftTags = idData ? DATA.allGifts.map(g => {
            const st = checkGiftStatus(g, p, i, stats);
            return st > 0 ? `<span class="${st===2?'gift-extra':'gift-active'} px-1.5 rounded text-[8px] mr-0.5 whitespace-nowrap">${g.name}</span>` : '';
        }).join('') : '';
...
function renderGifts() {
    ...
    cont.innerHTML = ''; const roman = { 1: 'I', 2: 'II', 3: 'III', 4: 'IV', 5: 'V' };
...
function renderFloors() {
    ...
    // 1. 현재 모든 층에서 선택된 팩 이름들을 수집 (중복 방지용)
    const selectedPacks = slots[activeIdx].floors.map(f => f.pack).filter(p => p !== "");

    for(let i=1; i<=15; i++) {
        const floorIndex = i - 1;
        const floorData = slots[activeIdx].floors[floorIndex];
        const packOptions = DATA.packs[i] || []; // 해당 층의 팩 목록
        
        // 현재 이 층에서 선택된 실제 팩 객체 찾기
        ...
        packOptions.forEach(pk => {
            ...
            const isSelectedElsewhere = selectedPacks.includes(pk.name) && pk.name !== floorData.pack;
            const selectedAttr = floorData.pack === pk.name ? 'selected' : '';
            const disabledAttr = isSelectedElsewhere ? 'disabled style="color: #444;"' : '';
            
            optionsHtml += `<option value="${pk.name}" ${selectedAttr} ${disabledAttr}>
                ${pk.name}${isSelectedElsewhere ? ' (선택됨)' : ''}
            </option>`;
        });

        list.innerHTML += `
            <div class="bg-stone-900 border border-stone-800 p-1.5 rounded shadow-inner">
                <div class="text-[10px] text-stone-500 font-bold mb-1 border-b border-stone-800 pb-0.5">${i}F</div>
                <select onchange="updateFloor(${floorIndex}, this.value)" class="w-full bg-black text-[11px] text-stone-300 outline-none mb-1 cursor-pointer">
                    ${optionsHtml}
                </select>
                <!-- [공란 방어 로직] 팩이 없거나 gifts가 없으면 빈칸 출력 -->
                <div class="text-[11px] text-yellow-400 leading-normal mt-1 font-bold">
                    ${(curPackObj && curPackObj.gifts && Array.isArray(curPackObj.gifts)) 
                        ? curPackObj.gifts.filter(g => g.trim() !== "").map(g => `• ${g}`).join('<br>') 
                        : ''}
                </div>
            </div>`;
    }
...
// [5] 시스템 유틸리티
function toggleSidebar(side) { const sb = document.getElementById(side + '-sidebar'), icon = document.getElementById(side + '-icon'); if(side === 'left') { isLCol = !isLCol; sb.classList.toggle('collapsed', isLCol); icon.className = isLCol ? "fas fa-chevron-right" : "fas fa-chevron-left"; } else { isRCol = !isRCol; sb.classList.toggle('collapsed', isRCol); icon.className = isRCol ? "fas fa-chevron-left" : "fas fa-chevron-right"; } }
function toggleReserve() { isResCol = !isResCol; const res = document.getElementById('reserve-party'); if(res) res.style.maxHeight = isResCol ? '0' : '1000px'; document.getElementById('reserve-icon').className = isResCol ? "fas fa-chevron-down" : "fas fa-chevron-up"; }
...
function copyExportCode() { const el = document.getElementById('out-code'); el.select(); document.execCommand('copy'); alert("복사되었습니다."); }
function importCode(asNew) { try { const data = JSON.parse(decodeURIComponent(atob(document.getElementById('in-code').value))); if(asNew) { slots.push(data); activeIdx = slots.length-1; } else slots[activeIdx] = data; document.getElementById('in-code').value = ''; refreshUI(); saveToLocal(); } catch(e) { alert("코드 오류"); } }
